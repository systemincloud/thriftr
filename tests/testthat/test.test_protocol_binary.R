#! /usr/bin/env Rscript

library(testthat)

context("type_protocol_binary")

newenv = function(inputs) {
  ret <- new.env(hash = TRUE)
  for (input in inputs) {
    ret[[input[[1]]]] <- input[[2]]
  }
  return(ret)
}

TItem <- R6::R6Class("TItem",
  inherit = thriftr::TPayload,
  public = list(
    thrift_spec = newenv(list(
      list("1", list(thriftr::TType$I32, 'id', FALSE)),
      list("2", list(thriftr::TType$LIST, 'phones', list(thriftr::TType$STRING), FALSE))
    )),
    default_spec = list(
      c('id', NA),
      c('phones', NA)
    )
  )
)

test_that("test_pack_i8", {
  b <- thriftr::binary_write_val(thriftr::TType$I08, 123)
  expect_equal(thriftr::hexlify(b), "7b")
})

test_that("test_unpack_i8", {
  b <- charToRaw("{")
  expect_equal(thriftr::binary_read_val(b, thriftr::TType$I08), 123)
})

test_that("test_pack_i16", {
  b <- thriftr::binary_write_val(thriftr::TType$I16, 12345)
  expect_equal(thriftr::hexlify(b), "30 39")
})

test_that("test_unpack_i16", {
  b <- charToRaw("09")
  expect_equal(thriftr::binary_read_val(b, thriftr::TType$I16), 12345)
})

test_that("test_pack_i32", {
  b <- thriftr::binary_write_val(thriftr::TType$I32, 1234567890)
  expect_equal(thriftr::hexlify(b), "49 96 02 d2")
})

test_that("test_unpack_i32", {
  b <- as.raw(strtoi(c("49", "96", "02", "d2"), 16L))
  expect_equal(thriftr::binary_read_val(b, thriftr::TType$I32), 1234567890)
})

test_that("test_pack_i64", {
  b <- thriftr::binary_write_val(thriftr::TType$I64, c(287445236, 2112454933))
  expect_equal(thriftr::hexlify(b), "11 22 10 f4 7d e9 81 15")
})

test_that("test_unpack_i64", {
  b <- as.raw(strtoi(c("11", "22", "10", "f4", "7d", "e9", "81", "15"), 16L))
  l <- thriftr::binary_read_val(b, thriftr::TType$I64)
  expect_equal(l[1], 287445236)
  expect_equal(l[2], 2112454933)
})

test_that("test_pack_double", {
  b <- thriftr::binary_write_val(thriftr::TType$DOUBLE, 1234567890.1234567)
  expect_equal(thriftr::hexlify(b), "41 d2 65 80 b4 87 e6 b7")
})

test_that("test_unpack_double", {
  b <- as.raw(strtoi(c("41", "d2", "65", "80", "b4", "87", "e6", "b7"), 16L))
  expect_equal(thriftr::binary_read_val(b, thriftr::TType$DOUBLE), 1234567890.1234567, tolerance = 1e-17)
})

test_that("test_pack_string", {
  b <- thriftr::binary_write_val(thriftr::TType$STRING, "hello world!")
  expect_equal(thriftr::hexlify(b), "00 00 00 0c 68 65 6c 6c 6f 20 77 6f 72 6c 64 21")

  b <- thriftr::binary_write_val(thriftr::TType$STRING, "你好世界")
  expect_equal(thriftr::hexlify(b), "00 00 00 0c e4 bd a0 e5 a5 bd e4 b8 96 e7 95 8c")
})

test_that("test_unpack_string", {
  b <- as.raw(strtoi(c("00", "00", "00", "0c", "e4", "bd", "a0",
    "e5", "a5", "bd", "e4", "b8", "96", "e7", "95", "8c"), 16L))
  expect_equal(thriftr::binary_read_val(b, thriftr::TType$STRING), "你好世界")
})

test_that("test_unpack_binary", {
  b <- as.raw(strtoi(c("00", "00", "00", "0c", "e4", "bd", "a0",
    "e5", "a5", "bd", "e4", "b8", "96", "e7", "95", "8c"), 16L))
  expect_equal(thriftr::binary_read_val(b, thriftr::TType$STRING, decode_response = FALSE), charToRaw("你好世界"))
})

test_that("test_write_message_begin", {
})

test_that("test_write_message_begin_not_strict", {
})

test_that("test_read_message_begin", {
})

test_that("test_read_message_begin_not_strict", {
})

test_that("test_write_struct", {
})

test_that("test_read_struct", {
})

test_that("test_write_empty_struct", {
})

test_that("test_read_empty_struct", {
})

test_that("test_write_huge_struct", {
})
